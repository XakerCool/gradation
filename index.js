...
const {addBxLink, getBxCredentials, setConnection, checkIfExists, addCompaniesToDb, addDealsToDb, setSummary, getLastDealDateFromSummary, getFromDb, getMaxId, markOnCall} = require("./services/db.js");
...
app.post("/gradation/set_and_return_current_data", async (req, res) => {
    try {
        ...
        return res.status(500).json({ "status": "error", "message": "Что-то пошло не так" });
    }
}, haltOnTimedOut);
